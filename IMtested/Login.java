...
class Login extends Frame implements ActionListener{
	...
	Login(){
		...
		Button bcl=new Button("注册");
		bcl.addActionListener(new ActionListener(){
			public void actionPerformed(ActionEvent e) {
				try {
					new Reg(new Socket(tsip.getText().trim(),8888));
				} catch (UnknownHostException e1) {
					JOptionPane.showMessageDialog(null,"服务器未知，请修改为正确的服务器ip");
	...
	public void login(){
		...
		try {
			//判断id不能为\w以外的字符 pw不能为空 ，否则立即返回 ，不执行后面的语句     还应判读长度 被省略
			if(!this.tid.getText().trim().matches("\\w+") || !tpw.getText().trim().matches("\\w+")
					|| tid.getText().trim().length()>20 || tpw.getText().trim().length()>10){
			...
			JOptionPane.showMessageDialog(this,"服务器未知，请修改为正确的服务器ip");
		} catch (IOException e1) {
			JOptionPane.showMessageDialog(this,"服务器未知，请修改为正确的服务器ip!");
		}
	}	
